...
## 抽象类与接口
### 1、抽象类
抽象类和抽象方法都是使用abstract关键字进行声明的。如果一个类中包含抽象方法，那么这个类必须声明为抽象类。（抽象类不一定要求一定要有抽象方法）
...
### 两者的比较与使用选择
一个类可以实现多个接口，但是不能继承多个抽象类。抽象类可以有构造函数，但不能实例化。接口没有构造函数，JDK8后接口默认方法和静态方法可以有方法体。抽象类表示的是is-a关系，强调的是从属关系；接口表示的是like-a关系，强调的是功能。
### 使用接口
- 需要让不相关的类都实现一个方法
- 需要使用多重继承
### 使用抽象类
...
- 静态方法：静态方法在类加载的时候就存在了，它不依赖于任何实例，所以静态方法必须有实现，也就是说它不能是抽象方法（只能访问所属类的静态字段和静态方法(要想访问实例方法必须创建类的实例对象再调用)，方法中不能有this和super关键字）
#
	public class A{
...
- 方法引用：可以直接引用已有Java类或对象的方法或构造器，与lambda联合使用。
- Stream API:函数式编程
